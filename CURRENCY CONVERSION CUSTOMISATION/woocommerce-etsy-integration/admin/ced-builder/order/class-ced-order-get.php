...
class Ced_Order_Get {
	...
	public function get_orders( $shopId, $is_sync = false ) {
        ...
        $result = @file_get_contents( CED_ETSY_DIRPATH . 'admin/lib/json/woo-etsy-test-order.json' );
		...
		if ( is_array( $orders ) && ! empty( $orders ) ) {
			...
			foreach ( $orders as $order ) {
					...
					if ( ! empty( $transactions_per_reciept ) ) {
						...
						foreach ( $transactions_per_reciept as $transaction ) {
							...
							if ( get_woocommerce_currency() == 'USD' ) {
								...
								$converted_price = $renderDataOnGlobalSettings[$shop_name]['product_data']['_ced_etsy_conversion_rate']['default'];
								$basePrice = (float)$basePrice/(float)$converted_price;
								$basePrice                                = !empty($basePrice) ? round( $basePrice, 2 ) : '';
				...
				if ( get_woocommerce_currency() == 'USD' ) {
					...
					$ShippingAmount = (float)$ShippingAmount/(float)$converted_price;
					$ShippingAmount                                = !empty($ShippingAmount) ? round( $ShippingAmount, 2 ) : '';
					$ShippingAmount = (float) $ShippingAmount;

					$DiscountedAmount = (float)$DiscountedAmount/(float)$converted_price;
					$DiscountedAmount                                = !empty($DiscountedAmount) ? round( $DiscountedAmount, 2 ) : '';
					$DiscountedAmount = (float) $DiscountedAmount;

					$gift_wrap_price = (float)$gift_wrap_price/(float)$converted_price;
					$gift_wrap_price                                = !empty($gift_wrap_price) ? round( $gift_wrap_price, 2 ) : '';
					$gift_wrap_price = (float) $gift_wrap_price;

					$finalTax = (float)$finalTax/(float)$converted_price;
					$finalTax                                = !empty($finalTax) ? round( $finalTax, 2 ) : '';
