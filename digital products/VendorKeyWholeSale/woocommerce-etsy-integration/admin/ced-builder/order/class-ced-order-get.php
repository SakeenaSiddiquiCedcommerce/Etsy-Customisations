...
class Ced_Order_Get {
	...
	public function get_orders( $shopId, $is_sync = false ) {
		...
		$create_order = isset($this->saved_global_settings_data[$shopId]['create_etsy_order_on_woocommerce']) ? $this->saved_global_settings_data[$shopId]['create_etsy_order_on_woocommerce'] : '';
         if($create_order != 'on' ){
            return;
         }
		...
		if ( isset( $result['results'] ) && ! empty( $result['results']  &&  $create_order == 'on')) {
