...
if ( ! class_exists( 'Ced_Etsy_Import_Products' ) ) {
	...
	class Ced_Product_Import {
		...
		private function create_simple_product( $product = '', $product_details = '', $image_details = '', $shop_name = '' ) {
			...
			if(!empty($product['file_data'])) {
				$downloads = array();
				update_post_meta( $product_id, '_downloadable', 'yes' );
				$shop_id      = get_etsy_shop_id( $shop_name );
				$digital_files = etsy_request()->get( "application/shops/{$shop_id}/listings/{$product['listing_id']}/files", $shop_name );
				foreach($digital_files['results'] as $digital_files_key => $digital_files_val) {
					if (class_exists('WC_Product')) {
					    ...
					    if ($existing_product) {
					        ...
					        //     $filedata   = file_get_contents($image_data);
                  //     // $file_name = wp_unique_filename(wp_upload_dir()['path'], basename($file_url));
                  //     // $file_name = $file_name.'.jpg';
                  //     $file_name    = basename($digital_files_val['filename']);
                  //     $file_path = wp_upload_bits($file_name, null, $image_data);
					        //     if (!$file_path['error']) {
					        //           $attachment = array(
					        //               'post_title' => sanitize_file_name(pathinfo($file_path['file'], PATHINFO_FILENAME)),
					        //               'post_content' => '',
					        //               'post_status' => 'inherit',
					        //           );

					        //           $attachment_id = wp_insert_attachment($attachment, $file_path['file']);
					        //           $attach_data = wp_generate_attachment_metadata($attachment_id, $file_path['file']);
					        //           wp_update_attachment_metadata($attachment_id, $attach_data);

					        //           echo 'Image uploaded successfully!';
					        //       } else {
					        //           echo 'Error uploading image: ' . $file_path['error'];
					        //       }

        					//         $download =
        					//             array(
        					//                 'name' => $digital_files_val['filename'],
        					//                 'file' => $file_path['url'],
        					...
        					//         $downloads[] = $download;
                  //         if(!empty($file_url)) {
                  //           $response[] = 'Digital Product impprted sucessfully';
                  //           $response[] = 'Product title : ' . $product['title'];
                  //           $response[] = 'Digital file name : ' . $file_name;
                  //           $response[] = 'Digital file url : ' . $file_url;
                  //           $final_response = array( 'response' => array( 'results' => $response ) );
                  //         } else {
                  //         	$response[] = 'Digiatl file missing from product.';
                  //         	$response[] = 'Product title : ' . $product['title'];
		        ...
		        global $activity;
		        $activity->action        = 'Importer';
		        $activity->type          = 'downloadable';
		        $activity->input_payload = $downloads;
		        $activity->post_id       = $product_id;
		        $activity->shop_name     = $shop_name;
		        $activity->is_auto       = false;
		        $activity->post_title    = $product['title'];
