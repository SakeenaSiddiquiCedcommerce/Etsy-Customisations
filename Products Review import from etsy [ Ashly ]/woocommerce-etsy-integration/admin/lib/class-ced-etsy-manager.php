	...
	class Ced_Etsy_Manager {
		...
		public function ced_etsy_schedules() {
			if ( isset( $_GET['shop_name'] ) && ! empty( $_GET['shop_name'] ) ) {
				...
				$ced_etsy_auto_import_reviews      = isset( $data_on_global_settings[ $shop_name ]['ced_etsy_auto_import_shop_reviews'] ) ? $data_on_global_settings[ $shop_name ]['ced_etsy_auto_import_shop_reviews'] : '';
				$ced_etsy_updt_invntry_etsy_to_woo = isset( $data_on_global_settings[ $shop_name ]['ced_etsy_auto_update_invenotry_From_etsy_to_woo'] ) ? $data_on_global_settings[ $shop_name ]['ced_etsy_auto_update_invenotry_From_etsy_to_woo'] : '';
				...
				if ( 'on' == $ced_etsy_auto_import_reviews && ! wp_get_schedule( 'ced_etsy_import_shop_review_to_woostore_' . $shop_name ) ) {
					wp_schedule_event( time(), 'ced_etsy_6min', 'ced_etsy_import_shop_review_to_woostore_' . $shop_name );
				}elseif( 'on' != $ced_etsy_auto_import_reviews){
					wp_clear_scheduled_hook( 'ced_etsy_import_shop_review_to_woostore_' . $shop_name );
				}
				
				// Update Woo Inventory from Etsy if already exists
				if ( 'on' == $ced_etsy_updt_invntry_etsy_to_woo && ! wp_get_schedule( 'ced_etsy_update_inventory_from_etsy_to_woostore_' . $shop_name ) ) {
					wp_schedule_event( time(), 'ced_etsy_10min', 'ced_etsy_update_inventory_from_etsy_to_woostore_' . $shop_name );
				}elseif( 'on' != $ced_etsy_updt_invntry_etsy_to_woo ) {
					wp_clear_scheduled_hook( 'ced_etsy_update_inventory_from_etsy_to_woostore_' . $shop_name );
				}
			}
		}
