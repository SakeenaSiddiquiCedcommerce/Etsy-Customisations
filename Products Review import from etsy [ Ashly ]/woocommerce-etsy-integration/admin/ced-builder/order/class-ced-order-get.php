		...
		if ( is_array( $orders ) && ! empty( $orders ) ) {
			...
			foreach ( $orders as $order ) {
				...
				// 			$CancelQty  = 0;
							...
							/**
							 *=======================================
							 * UPDATE STOCK WITHOUT CREATING ORDERS 
							 *=======================================
							 */
							...
							if ( $ID && is_object( wc_get_product( $ID ) ) && 'yes' != $stock_reduced ) {
								...
								update_post_meta( $ID, '_ced_etsy_stock_reduced_' . $OrderNumber, 'yes' );
						  	    $update_stock_response['results'][] = "Stock was updated on product ID :- " .$ID . " and product title :- " . get_the_title( $ID );
							}else{
							    if( "yes" == $stock_reduced ){
							        $update_stock_response['results'][] = "Product stock already reduced before not need to reduce the stock | product id " . $ID . " and product listing ID " . $listing_id . "product title :- " . get_the_title( $ID );
							    }else{
							        $update_stock_response['results'][] = "Product was not found on WooCommerce with product id " . $ID . " and product listing ID " . $listing_id;
							    }
				...
				// 			$ItemArray[] = $item;
					...
					$activity->response = $update_stock_response;
					$activity->execute();
				}
                continue;
