...
if ( ! class_exists( 'Ced_Etsy_Import_Products' ) ) {
	...
	class Ced_Product_Import {
		...
		/**
		 * ***************************************************
		 * Create review as WooCommerce comment on WordPress
		 * ***************************************************
		 *
		 * @since    2.0.0
		 */
		...
		public function ced_etsy_create_comment_to_post( $post_id=0, $review_data=array(), $shop_name='', $listing_id ='' ){
			
			global $wpdb;
			$author_name = isset( $review_data['buyer_name'] ) ? $review_data['buyer_name'] : 'Unknown';
			$e_review    = isset( $review_data['review'] ) ?  $review_data['review'] : '';
			...
			$comment_ids = $wpdb->get_col(
				$wpdb->prepare(
					...
					AND c.comment_content = %s
					AND cm.meta_key = 'rating'
                                                    AND cm.meta_value = %s", // Added condition to check comment meta.
                                                    $post_id,
                                                    $author_name,
                                                    $e_review,
                                                    $rating
         ...
         // // $comment_ids = !is_array( $comment_ids ) ? array( $comment_ids ) : $comment_ids
			if(!empty($comment_ids)){
				global $activity;
				$activity->action        = 'Import Review';
				$activity->type          = 'product_review';
				$activity->input_payload = $post_id . '___Update Reviews Logs';
				
				foreach( $comment_ids as $key => $comment_id ){
					if( $comment_id ){
						$comment = get_comment($comment_id);
						if ($comment) {
							$comment->comment_date = isset( $review_data['created_timestamp'] ) ? date('Y-m-d H:i:s', $review_data['created_timestamp'] )  : date( 'Y-m-d H:i:s' );
							$comment->comment_date_gmt = isset( $review_data['created_timestamp'] ) ? date('Y-m-d H:i:s', $review_data['created_timestamp'] )  : date( 'Y-m-d H:i:s' );
							// var_dump($comment);
							/*die("tet");*/
                    		...
                    		if( empty( $comment_ids ) || is_null( $comment_ids ) || '' == $comment_ids ) {
                    			$review_id = wp_insert_comment(
                    				array(
                    					'comment_post_ID'      => $post_id,
                    					'comment_author'       => isset( $review_data['buyer_name'] ) ? $review_data['buyer_name'] : 'Unknown',
                    					'comment_author_email' => '',
                    					'comment_author_url'   => '',
                    					'comment_content'      => isset( $review_data['review'] ) ?  $review_data['review'] : '',
                    					'comment_type'         => 'review',
                    					'comment_parent'       => 0,
                    					'user_id'              => 0,
                    					'comment_author_IP'    => '',
                    					'comment_agent'        => '',
                    					'comment_date'         => isset( $review_data['created_timestamp'] ) ? date('Y-m-d H:i:s', $review_data['created_timestamp'] )  : date( 'Y-m-d H:i:s' ),
                    					'comment_date_gmt'     => isset( $review_data['created_timestamp'] ) ?  date('Y-m-d H:i:s', $review_data['created_timestamp'] ) : date( 'Y-m-d H:i:s' ),
                    					'comment_approved'     => 1,
                    					'comment_meta'         => array(
                    						'_ced_etsy_listing_id_'. $shop_name => isset( $review_data['listing_id'] ) ? $review_data['listing_id'] : $listing_id,
                    						'_ced_etsy_shop_name'  => $shop_name,
                    					),
                    				)
                    			);
		...
		/**
		 * ********************************************************
		 * Get buyer from the Etsy create customer in WooCommerce.
		 * ********************************************************
		 *
		 * @since    2.0.0
		 */

		public function ced_etsy_get_buyer_from_etsy( $user_id, $shop_name ) {
			do_action( 'ced_etsy_refresh_token', $shop_name );
			$etsy_buyer = etsy_request()->get( "application/users/{$user_id}", $shop_name );
			return $etsy_buyer;
		}
		...
		/**
		 * *************************************
		 *  Get reviews from the Etsy listing
		 * *************************************
		 *
		 * @since    2.0.0
		 */
		public function ced_etsy_get_reviews_by_listing_id( $shop_name, $post_id, $listing_id ) {
			$listing_reviews = array();
			$review_offset   = 0;
			if (get_post_meta( $post_id, 'ced_etsy_import_review_offeset_' . $shop_name, true ) ) {
				$review_offset = get_post_meta($post_id, 'ced_etsy_import_review_offeset_' . $shop_name, true );
			}
			$per_page = 50;
			$params = array(
			...
			do_action( 'ced_etsy_refresh_token', $shop_name );
			$reviews = etsy_request()->get( "application/listings/{$listing_id}/reviews", $shop_name, $params );
			if ($reviews['count']) {
				$listing_reviews = isset( $reviews['results'] ) ? $reviews['results'] : array();
				update_post_meta( $post_id, 'ced_etsy_import_review_offeset_' . $shop_name, ( $review_offset + $per_page ) );
				return $listing_reviews;
			}
			return array();
		}

		public function ced_etsy_import_review_by_listgid_n_create_woo_comnt( $shop_name, $post_id, $listing_id ) {
			global $activity;
			$activity->action        = 'Import Review';
			$activity->type          = 'product_review';
			$activity->input_payload = array( "shop_name" => $shop_name, "post_id" => $post_id, "listing_id" => $listing_id);
			$all_e_reviews           = array();
			$get_etsy_reviews        = $this->ced_etsy_get_reviews_by_listing_id( $shop_name, $post_id, $listing_id );
			$all_e_reviews           = isset( $get_etsy_reviews['results'] ) ? $get_etsy_reviews['results'] : $get_etsy_reviews;
			$activity->response      = $all_e_reviews;
			foreach( $all_e_reviews  as $e_r_key => $e_reviews ){
				$this->ced_etsy_create_comment_to_post( $post_id, $e_reviews, $shop_name, $listing_id );
			}
			$activity->post_id       = $post_id;
			$activity->shop_name     = $shop_name;
			$activity->is_auto       = false;
			$activity->post_title    = get_the_title( $post_id );
			$activity->execute();
		}

		/**
		 * ********************************
		 * Assign WC comment to posts 
		 * ********************************
		 *
		 * @since    2.0.0
		 */
		public function ced_etsy_assgin_wc_cmnt_to_post( $listing_id = '', $shop_name = '', $product_id ='' ) {
			if (empty($product_id)) {
				return false;
			}
			$pr_comments = get_comments( array( 'meta_key'=> '_ced_etsy_listing_id_' .$shop_name , 'meta_value'=> $listing_id ) );
			if ($pr_comments) {
				foreach( $pr_comments as $wc_revws ) {
					$commentarr                    = array();
					$commentarr['comment_ID']      = $wc_revws->comment_ID;
					$commentarr['comment_post_ID'] = $product_id;
		...
		private function ced_etsy_get_listing_infos( $etsy_product = array(), $shop_name = '' ) {
			...
			$woo_pro_id          = get_product_id_by_params( '_ced_etsy_listing_id_' . $shop_name, $listing_id );
			if ( ! empty( $etsy_product_exists ) || $woo_pro_id ) {
				return;
			}
			/** Refresh token
				 *
			...
			if ( isset( $inventories['products'] ) && count( $inventories['products'] ) > 1 ) {
			    $this->manage_var_stock = !empty($inventories['quantity_on_property']) ? true : false;
		...
		private function create_simple_product( $product = '', $product_details = '', $image_details = '', $shop_name = '' ) {
			...
			/**
			 **************************************************
			 * UPDATE COMMENT TO A PRODUCT IF ALREADY CREATED 
			 **************************************************
			 */
			$res = $this->ced_etsy_assgin_wc_cmnt_to_post( $product['listing_id'], $shop_name, $product_id );
			// $this->ced_etsy_import_review_by_listgid_n_create_woo_comnt( $shop_name, $product_id, $product['listing_id'] );
		...
		private function create_variable_product( $product = '', $product_details = '', $image_details = '', $shop_name = '' ) {
			...
			foreach ( $etsy_variation_products as $key => $value ) {
				...
				//	$variations[ $key ]['stock_manage'] = true;
