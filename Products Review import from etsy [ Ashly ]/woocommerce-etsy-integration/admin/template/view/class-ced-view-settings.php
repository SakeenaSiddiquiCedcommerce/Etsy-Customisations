...
// 		do_action( 'ced_etsy_import_shop_review_to_woostore_' . $this->shop_name );
	}
	...
	public function ced_etsy_save_settings() {
		...
		if ( ! empty( $auto_import_schedule ) ) {
			wp_schedule_event( time(), 'ced_etsy_15min', 'ced_etsy_auto_import_schedule_job_' . $this->shop_name );
	...
	public function ced_etsy_all_settings_fields() {
		return array(
			...
			'scheduler_setting_view' => array(
				array(
					'label'   => __( 'Update stock without fetching order from Etsy to WooCommerce Scheduler', 'woocommerce-etsy-integration' ),
					'tooltip' => 'Auto update stock if order made purchased on Etsy.Com without creating the order into WooCommerce.',
					'type'    => 'check',
				...
				array(
					'label'   => __( 'Import Products And Assign Reviews', 'etsy-woocommerce-integration' ),
				...
				array(
					'label'   => __( 'Auto Import Shop Reviews', 'etsy-woocommerce-integration' ),
					'tooltip' => 'Auto import the active listings from etsy to woocommerce.',
					'type'    => 'check',
					'name'    => 'ced_etsy_auto_import_shop_reviews',
					'options' => '',
				),
				array(
					'label'   => __( 'Update Inventory from Etsy to WooCommerce', 'etsy-woocommerce-integration' ),
					'tooltip' => 'This option will help you to update the inventory from Etsy to WooCommerce if the product already existing in WooCommerce store.',
					'type'    => 'check',
					'name'    => 'ced_etsy_auto_update_invenotry_From_etsy_to_woo',
					'options' => '',
				),
			),
		);
