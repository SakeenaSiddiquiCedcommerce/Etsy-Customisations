...
<div class="ced_etsy_timeline_wrap">
	...
	$log_types = array(
		...
		'Order ( Update stock without creating order to WooCommerce )'     => 'ced_etsy_order_logs_' . $shop_name,
		'Product'   => 'ced_etsy_product_logs_' . $shop_name,
		'Review Import'   => 'ced_etsy_product_review_logs_' . $shop_name,
		'Inventory Update from Etsy to WooCommerce ' => 'ced_etsy_inventory_update_from_e_to_woo_logs_' . $shop_name,
		'Auto Import Product from Etsy to WooCommerce Cron Jobs ' => 'ced_auto_import_cron_job_' . $shop_name,
		
	);
// 	delete_option('ced_etsy_order_logs_' . $shop_name);
	...
	<div class="ced_etsy_timeline_body">
			...
			<?php 
			    if($log_type == 'ced_etsy_inventory_update_from_e_to_woo_logs_'  .$shop_name ){
			        $total_details = get_option('ced_pregress_total_number_product_have_updated_'.$shop_name, 0);
			        $offset_e      = get_option('ced_etsy_update_inventory_from_e_to_woo_offset_'.$shop_name, 0 );
			     //   $updated_rec   = $total_details - $offset_e;
			        echo "<b>The total updated products from the Etsy to WooCommerce are : ( " .$offset_e. " out of " .$total_details. " ) </b>";
			    }
			?>
	...
	<div class="ced_etsy_timeline_body">
		...
		$etsy_events = array(
			...
			'Auto import reviews  cron job'   => 'ced_etsy_import_shop_review_to_woostore_' . $shop_name,
			'Auto Update Invetory from Etsy to Woo Cron job' => 'ced_etsy_update_inventory_from_etsy_to_woostore_' . $shop_name,
		);
		echo '<ul>';
