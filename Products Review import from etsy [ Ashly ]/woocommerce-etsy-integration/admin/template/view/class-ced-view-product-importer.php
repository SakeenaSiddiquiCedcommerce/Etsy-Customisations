...
class EtsyListImportedProducts extends WP_List_Table {
   ...
   */
	...
	public function column_cb( $item ) {
		...
		// } else {
			return sprintf(
				'<input type="checkbox" name="etsy_import_products_id[]" class="etsy_import_products_id" value="%s" data-woo_product_id="%s"/>',
				$item['listing_id'], $if_product_exists,
			);
		}else{
			return sprintf(
	...
	public function column_import_status( $item ) {
		...
		if ( ! empty( $if_product_exists ) ) {
			// update_option( 'ced_product_is_availabe_in_woo_' . $item['listing_id'], $item['listing_id'] );
			$image_path_imported = CED_ETSY_URL . 'admin/assets/images/check.png';
			return sprintf( '<img class="check_image" src="' . $image_path_imported . '" alt="Done">' );
		}else{
			$image_path_not_imported = CED_ETSY_URL . 'admin/assets/images/cross.png';
			return sprintf( '<img class="cross_image" src="' . $image_path_not_imported . '" alt="Done">' );
		} 
	...
	public function get_columns() {
		$columns = array(
			...
			'import_status' => __( 'Import Status', 'woocommerce-etsy-integration' ),
		);
		return $columns;
	}
		...
		<div id="post-body" class="metabox-holder columns-2">
			<div id="post-body-content">
				<div class="meta-box-sortables ui-sortable">
					...
					/**
					 *==============================
					 *  IMPORT REVIEW PROCESS START
					 *==============================
					 */
					// echo '<button class="button-primary" id="ced_etsy_fetch_shop_reviews" data-shop_name="' . esc_attr( $shop_name ) . '" >' . esc_html( __( 'Fetch Shop Reviews', 'woocommerce-etsy-integration' ) ) . '</button>';
					// $e_shop_revies = get_comments( array( 'meta_key'=> '_ced_etsy_shop_name' , 'meta_value'=> $shop_name ) );
					// $e_shop_revies = !empty( $e_shop_revies ) ? $e_shop_revies : array();
					// echo " <b> Total imported Etsy shop reviews :-  " . count( $e_shop_revies ) . "</b>";

					/**
					 *============================
					 *  IMPORT REVIEW PROCESS END
					 *============================
					 */
					...
					<form method="post">
						<?php
						$this->search_box('Search', 'search','Search Product By Listing Id');
						$this->display();
						?>
					</form>
				</div>
