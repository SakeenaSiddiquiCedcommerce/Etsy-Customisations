(function( $ ) {
		...
		$( document ).on(
			...
			function(e){
				...
				if ( operation <= 0 ) {
					...
					if(operation == 'import_review'){
						let etsy_import_products_id = new Array();
						let etsy_not_imported_product = new Array();
						$( '.etsy_import_products_id:checked' ).each(
							function(){
								if(undefined != $(this).attr('data-woo_product_id')) {
									etsy_import_products_id.push( $( this ).val() );
								} else {
									etsy_not_imported_product.push( $( this ).val() );
								}
							}
							);
						...
						$.ajax(
						{
							...
							data : {
								ajax_nonce   : ajaxNonce,
								action       : 'ced_etsy_fetch_shop_reviews_from_etsy_to_woo',
							...
							success : function( response ){
								...
								var not_imported_listing_ids = '';
								if(etsy_not_imported_product.length > 0){
									not_imported_listing_ids += "Please Import these '" + etsy_not_imported_product+ "' listing first on woo then you can import the reviews ";
									
								notice += "<div class='" + classes + "'>" + not_imported_listing_ids + "</div>";
								}
								notice += "<div class='" + classes + "'>" + parsed_response.message + "</div>";
								// $( '.ced_etsy_loader' ).show();
								$( ".success-admin-notices" ).append( notice );
	...
	function performBulkAction( etsy_products_id,operation ,total_products)
		...
		// $( document ).on(
		// 	'click',
		// 	'#ced_etsy_fetch_shop_reviews',
		// 	function(e){

		// 		e.preventDefault();

		// 		let operation_to_be_performed               = $( '.bulk-import-action-selectorf' ).val();
		// 			let etsy_import_products_id = new Array();
		// 			// Get all checked ids in the array from input box
		// 			$( '.etsy_import_products_id:checked' ).each(
		// 				function(){
		...
		// 			);
		// 			// importProductsFromEtsy( etsy_import_products_id , operation );
		// 			alert(operation_to_be_performed);
		...
		// 					ajax_nonce   : ajaxNonce,
		// 					action       : 'ced_etsy_fetch_shop_reviews_from_etsy_to_woo',
		// 					operation_to_be_performed :	operation_to_be_performed,
		// 					listing_id :	etsy_import_products_id,
		// 					shop_name    : shop_name,
		// 				},
		// 				type : 'POST',
		...
		// 						classes = 'notice notice-error';
		// 					}
		// 				   notice += "<div class='" + classes + "'>" + response.message + "</div>";
		// 				   $( '.ced_etsy_loader' ).show();
		// 				   $( ".success-admin-notices" ).append( notice );
		// 				   window.setTimeout( function(){window.location.reload()}, 2500 );
		// 				}
