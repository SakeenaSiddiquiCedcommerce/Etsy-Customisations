...
class Woocommmerce_Etsy_Integration {
	...
	private function define_admin_hooks() {
		...
		$ced_ajaxs = array(
			...
			'ced_etsy_fetch_shop_reviews_from_etsy_to_woo',
		...
		if ( ! empty( $shops ) ) {
			foreach ( $shops as $key => $value ) {
				if ( isset( $value['details']['ced_shop_account_status'] ) && 'Active' == $value['details']['ced_shop_account_status'] ) {
					...
					$this->loader->add_action( 'ced_etsy_import_shop_review_to_woostore_' . $key, $plugin_admin, 'ced_etsy_import_shop_reviews_sclr_cl_bck' );
					$this->loader->add_action( 'ced_etsy_auto_upload_products_' . $key, $plugin_admin, 'ced_etsy_auto_upload_products' );
					$this->loader->add_action('ced_etsy_update_inventory_from_etsy_to_woostore_' . $key, $plugin_admin, 'ced_etsy_update_inventory_from_etsy_to_woostore_mngr');
		...
		$this->loader->add_action( 'ced_etsy_import_shop_review_to_woostore_', $plugin_admin, 'ced_etsy_import_shop_reviews_sclr_cl_bck' );
		$this->loader->add_action('ced_etsy_update_inventory_from_etsy_to_woostore_', $plugin_admin, 'ced_etsy_update_inventory_from_etsy_to_woostore_mngr');
		$order_status = array(
			'new_order',
